...
class ViajesService {
  ...
  Future<List<Viaje>> buscarViajes({String? origen, String? destino, String? fecha}) async {
    ...
    final res = await client.dio.get('/viajes/', queryParameters: params);
    final body = res.data;
  ...
  Future<({Viaje viaje, List<PisoConfig> pisos})> getAsientos(int viajeId) async {
    final res = await client.dio.get('/viajes/$viajeId/asientos/');
